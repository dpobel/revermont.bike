...
function generateProfile(fileObj, files, conf, done) {
    var data = [], ele;
    ...
    jsdom.env({
        html: '<html><body><div></div></body></html>',
            ...
            require('./utils/classlist')(window.self);
...
function parseGpx(files, filePath, basePath, opts, done) {
    ...
    fs.readFile(basePath + '/' + fileObj.gpx, function (err, content) {
        ...
        generateProfile(fileObj, files, opts.profile, done);
    });
}
        ...
        Object.keys(files).forEach(function (filePath) {
            ...
            if ( files[filePath].gpx ) {
                tasks.push(function (cb) {
                    parseGpx(
                        files, filePath,
                        metalsmith.source() + '/' + path.dirname(filePath), {
