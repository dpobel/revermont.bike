...
function parsePoints(fileObj, doc, trackPoints, elevationMinStep) {
    var foundBounds,
        bounds = {maxlat: -90, minlat: 90, maxlon: -90, minlon: 90},
        points = [],
        distance = 0, prev,
    ...
    trackPoints.forEach(function (point) {
        ...
        if ( prev ) {
            ...
            if ( Math.abs(diff) > elevationMinStep ) {
                if ( diff > 0 ) {
                    elevation.gain += diff;
                } else {
...
function parseGpx(files, filePath, basePath, elevationMinDiff, done) {
    ...
    fs.readFile(basePath + '/' + fileObj.gpx, function (err, content) {
        ...
        parsePoints(fileObj, doc, trackPoints, elevationMinDiff);
        ...
        Object.keys(files).forEach(function (filePath) {
            ...
            if ( files[filePath].gpx ) {
                tasks.push(function (cb) {
                    parseGpx(files, filePath, metalsmith.source() + '/' + path.dirname(filePath), elevationMinDiff, cb);
                });
            }
        });
